...
# 특성(피처) 중 숫자형과 범주형(문자형 구분) - 서로 다른 전처리가 필요하기 때문
...
def build_pipeline() -> Pipeline:
    ...
    numeric_pipe = Pipeline([
        ...
        # 표준화로 스케일링 : monthly_fee(수만원 단위), complaints(0~5 정도의 단위)처럼 단위차이가 클 때
    ...
    category_pipe = Pipeline([
        ...
        #   - 사이킷런의 OneHotEncoder는 학습 시점의 규칙을 기억한다
        #     벡터로 처리, 파이프라인 단위로 할 때 편리하다
        #   - handle_unknown='ignore' : 처음 보는 값이 들어오면, 새 컬럼을 만드는 대신 설정에 따라 모든 값이 0인 벡터로 처리
        ('onehot', OneHotEncoder(handle_unknown='ignore'))
    ])

    # --- 컬럼별로 다른 전처리 동시에 적용 ---
    # ColumnTransformer : "이 열은 numeric_pipe로, 저 열은 category_pipe로 한 번에 지정해서 병렬 처리"
...
def train() -> dict:
    ...
    # predict_proba() : 각 클래스(0, 1)에 속할 확률 반환
    #                   [:, 1] 모든 행, 1번 컬럼
    #                   이탈할 확률
    probabilities = pipeline.predict_proba(X_test)[:, 1]

    # 확률이 0.5 이상이면 이탈(1), 미만이면 비이탈(0)로 최종 분류
    predictions = (probabilities >= 0.5).astype(int)

    # 정밀도(precision), 재현율(recall), f1-score 등을 클래스별로 계산 → 보고서
    #   output_dict=True : 딕셔너리 형태로 받아서 이후 코드에서 값을 꺼낼 때 쓰기 쉽게 한다
    ...
    metrics = {
        ...
        'roc_aur':round(float(roc_auc_score(y_test, probabilities)), 4),
