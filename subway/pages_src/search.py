'''
bigdata2026-basic/subwaypages_src/search.py

작성자 : 노가희
작성일 : 2026-08-06
목적 :
    - 폼 안에 입력 위젯들(역 이름, 승하 구분, 기간, 시작시 범위, 요일)
        → 조건들을 조작할 때마다 재실행되지 않고 "검색" 버튼을 누르는 순간에만 한 번에 처리되도록 폼을 사용
    - 만약에 데이터가 많을 때 폼 없이 위젯을 조작하면 필터링을 다시 했을 때 체감 속도가 느려짐
        → st.form 사용
'''
...
if submitted:
    ...
    if keyword:
        # filtered['역명'].str.contains : 문자열의 일부만도 검색
    ...
    # 슬라이더는 (최소값, 최대값) 튜플 반환 → 시작시가 그 범위 안에 있는 행만 남김
    filtered = filtered[
        (filtered['시작시'] >= hour_range[0]) &
        (filtered['시작시'] <= hour_range[1])
    ]

    if weekday_pick:
        filtered = filtered[filtered['요일코드'].isin(weekday_pick)]

    st.session_state.last_search_result = filtered  # about.py에서 확인할 수 있도록 세션에 저장
