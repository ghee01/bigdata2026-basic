'''
bigdata2026-basic/subwaypages_src/explore.py

작성자 : 노가희
작성일 : 2026-08-06
목적 :
    - 즐겨찾는 역 저장 → 다른 위젯을 조작해도 사라지지 않게 한다
...
st.title('📊 역별 탐색 (즐겨찾기 유지)')

# 데이터 불러오기
df = load_subway()
station_list = sorted(df['역명'].unique())

# 'favorite_stations' 키가 없을 때만 빈 리스트로 초기화
# (이 세션에서 처음 방문했을 때만)
...
col_add, col_btn = st.columns([3, 1])   # 3:1
with col_add:
...
with col_btn:
    ...
    if st.button('+ 추가'):
        if st.session_state.station_to_add not in st.session_state.favorite_stations:
...
if not st.session_state.favorite_stations:
    st.info('아직 즐겨찾기 한 역이 없습니다. 위에서 역을 선택하고 추가 버튼을 눌러보세요.')
else:
...
if st.session_state.favorite_stations:
    ...
    fav_df = df[df['역명'] == focus_station]

    st.subheader(f'{focus_station} - 승차/하차 합계')
    # observed=True : 실제 존재하는 조합만 그룹핑
    ...
    # (문자열인 시간대컬럼으로 정렬하면 "10시-11시"가 "1시-2시"보다 앞에 오는 문자열 정렬 오류가 생김)
