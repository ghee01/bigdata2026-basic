'''
bigdata2026-basic/subway/main.py
...
#   - 파일경로는 main.py 기준 "상대경로"
#   - default=True : 앱을 처음 켰을 때 보이는 화면
...
about_page = st.Page('pages_src/about.py', title='소개', icon='ℹ️')

# --- 사이드바에 페이지 연결 : 각 섹션 제목이 함께 표시 / 딕셔너리 형태 ---
pg = st.navigation({
    '메인':[home_page],
...
# pg.run() : 사용자가 사이드바에서 클릭한 페이지의 코드를 실제로 실행
#   - 코드 아래로는 아무것도 입력되어 있지 않아야 한다
#   - 실제 화면 구성은 각 페이지 파일이 담당
pg.run()